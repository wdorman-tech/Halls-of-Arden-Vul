---
{"dg-publish":true,"permalink":"/ancestry-houserules/","dg-note-properties":{"aliases":["Ancestry Houserule"],"categories":["[[houserule]]","[[arden-vul]]"],"created":"05-05-2026 12:00"}}
---


## Notes

Houserules covering ancestries, languages associated with ancestry, and human cultural choice.

## The Rule

**No maximum levels.** Demihuman ancestries no longer cap at predetermined levels — your character keeps progressing.

**Humans compensated.** Humans automatically get the maximum value on their first-level hit points (since they lose the demihuman level-cap edge in this campaign).

**Gnomes do not exist.** The [[Imperial Goblin\|Imperial Goblin]] ancestry uses Gnome statistics, except they learn an additional language **instead of** Gnomish. Anyone who would gain Gnomish from their ancestry instead learns an additional language of their choice.

**Humans select a culture.** Every human PC must select one of the available [[Culture\|cultures]]: [[Archontean\|Archontean]], [[Thorcin\|Thorcin]], or [[Wiskin\|Wiskin]]. Humans gain the **culture's language**, their **alignment tongue**, and **one other language** of their choice.

**Language swaps.** Any character may replace any starting language they receive with one of: [[Archontean (language)\|Archontean (language)]], [[Thorcin (language)\|Thorcin (language)]], [[Wiskin (language)\|Wiskin (language)]], or [[Mithric\|Mithric]] (the ancient language of magic researchers).

**Common does not exist.** Anyone who would learn [[Common\|Common]] instead learns another language of choice.

## Why this matters

- Removing level caps + giving humans max-HP-1 balances ancestries upward together — no one is the obvious "weak" pick.
- Gnomes-as-Imperial-Goblins reflavours one of the OSRIC ancestries to fit the setting's politics.
- Human culture choice fuses race and culture for humans the way OSRIC fuses them for demihumans, and it forces players to declare a relationship to the empire.
- The language swap and "no Common" rules together make language a meaningful choice rather than an autopilot.

## Related

- [[OSRIC\|OSRIC]] — base rules modified
- [[Imperial Goblin\|Imperial Goblin]]
- [[Culture\|Culture]] — [[Archontean\|Archontean]], [[Thorcin\|Thorcin]], [[Wiskin\|Wiskin]]
- [[Common\|Common]] (does not exist)
- [[Mithric\|Mithric]]
- [[The Halls of Arden Vul Player Doc\|The Halls of Arden Vul Player Doc]] — source
