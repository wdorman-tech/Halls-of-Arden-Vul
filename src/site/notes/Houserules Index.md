---
{"dg-publish":true,"permalink":"/houserules-index/","dg-note-properties":{"aliases":["Houserules","Arden Vul Houserules"],"categories":["[[organization]]","[[arden-vul]]","[[houserule]]"],"created":"05-05-2026 12:00"}}
---


## Notes

Every modification the GM has made to the base [[OSRIC\|OSRIC]] rules. Houserules are sourced from [[The Halls of Arden Vul Player Doc\|The Halls of Arden Vul Player Doc]] and may evolve as the campaign progresses.

## Inventory

- **No carry-weight system.** OSRIC's pound-based encumbrance is not used; carrying capacity is not tracked, and there are no movement or surprise penalties from carried items. Item weights in pounds are kept on equipment notes as a descriptive property only.
- [[Slot-Based Inventory\|Slot-Based Inventory]] — items are placed on a slot grid as an organisational aid. Pack animals (e.g., [[Mule\|Mule]]) get their own grids (mules: 6 × 4 = 24 slots).
- Container effects: [[Backpack\|Backpack]], [[Quiver\|Quiver]], [[Flask\|Flask]], [[Writing Satchel\|Writing Satchel]], [[Pouch\|Pouch]], [[Sack\|Sack]], [[Scroll Case\|Scroll Case]] each have their own slot-encompassing rules.

## Character Creation

- [[Modified Character Creation\|Modified Character Creation]] — 3d6 in order, swap any two scores after rolling.
- Ancestries: max levels removed; [[Human\|Human]] gets max HP at 1st level; [[Gnome\|Gnome]] does not exist — replaced by [[Imperial Goblin\|Imperial Goblin]]; [[Common\|Common]] does not exist; [[Mithric\|Mithric]] is added as a setting language.
- [[Human\|Human]]s must select a [[Culture\|Culture]] ([[Archontean\|Archontean]] / [[Thorcin\|Thorcin]] / [[Wiskin\|Wiskin]]).
- Class: alignment restrictions removed (except [[Cleric\|Cleric]] must match deity); [[Multi-Classing\|Multi-Classing]] and [[Dual-Classing\|Dual-Classing]] not used at start of campaign.
- [[Magic-User\|Magic-User]]s automatically know [[Read Magic\|Read Magic]] in addition to two random + two chosen spells, and must choose one of the [[Seven Collegia\|Seven Collegia]].
- [[Cleric\|Cleric]]s must select a deity from [[The Ten\|The Ten]] or [[The Seventeen\|The Seventeen]].
- Starting rumours are tracked **per player, not per character**: when a player rolls their *first* character they roll 1d3 [[Adventuring Rumour\|Adventuring Rumour]] + 1d2 [[Historical Rumour\|Historical Rumour]], record them on the player profile, and every subsequent PC that player runs walks in already knowing the same set. Backup characters do not roll new rumours.

## XP & Leveling

- [[Modified XP and Leveling\|Modified XP and Leveling]] — gold no longer awards XP. No mandatory training to level up; once you have the XP and rest in a safe location, you advance.
- [[Exploration Awards\|Exploration Awards]] — Tasks (minor 2% / major 5% / extraordinary 10% of party TXP) reward exploration and creativity.

## Downtime

- [[Downtime\|Downtime]] — overview.
- [[Training (Downtime)\|Training (Downtime)]] — 1 XP per GP, max 1000/wk.
- [[Connecting (Downtime)\|Connecting (Downtime)]] — 0.5 XP per GP, builds town reputation.
- [[Carousing\|Carousing]] — 1 XP per GP with a d6+d6 effect roll.
- [[Recuperating (Downtime)\|Recuperating (Downtime)]] — double recovery rate.

## Related

- [[OSRIC\|OSRIC]] — base ruleset
- [[The Halls of Arden Vul Player Doc\|The Halls of Arden Vul Player Doc]] — source of all houserules
- [[Full Vault Wiki and Formatting\|Full Vault Wiki and Formatting]] — vault overview
